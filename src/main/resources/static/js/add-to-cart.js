...
function initFavButtons(favButtons){
    // alert(Array.isArray(favButtons))
    for (let i = 0; i <favorites.length ; i++) {
        // alert((cart[i]).id)
        const index =  favButtons.findIndex(b=>parseInt(b.dataset.id)===cart[i].id);
        if (index>-1){
            favButtons[index].classList.add('added')
            favButtons[index].style.color='red'
        }
    }
}
